...
int main() {
  ...
  const auto test = [&](const shared_ptr<Gate>& circuit, const vector<int>& attributes, const bool& expected_ok) {
    static int i = 0;
    const GT pt(e, false);
  ...
  test(v1_node4, {0, 1, 2}, true);
  test(v1_node4, {0, 2}, false);
  ...
  test(v2_node5, {3, 0}, true);
  test(v2_node5, {0, 5}, false);
  test(v2_node5, {3, 0, 5, 4}, true);
  test(v2_node5, {3, 5, 1}, false);
  test(v2_node5, {1, 4}, false);
  return 0;
}
